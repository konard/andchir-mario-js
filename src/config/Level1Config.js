// Level 1 Configuration
// This structure makes it easy to add new levels

export default {
    name: 'WORLD 1-1',
    width: 6400, // Level width in pixels
    height: 600,
    backgroundColor: '#5c94fc',

    // Player starting position
    playerStart: {
        x: 100,
        y: 400
    },

    // Ground platforms
    grounds: [
        // Main ground
        { x: 0, y: 568, width: 3200, height: 32 },
        // Large gap for moving platforms (from 3200 to 3800 = 600 pixels)
        { x: 3800, y: 568, width: 600, height: 32 },
        // Second large gap for moving platforms (from 4400 to 5000 = 600 pixels)
        { x: 5000, y: 568, width: 200, height: 32 },
        // Floating platform
        { x: 800, y: 400, width: 192, height: 32 },
        { x: 1200, y: 350, width: 128, height: 32 },
        { x: 1500, y: 300, width: 128, height: 32 },
        { x: 5100, y: 568, width: 1500, height: 32 }
    ],

    // Brick blocks
    bricks: [
        { x: 400, y: 400 },
        { x: 432, y: 400 },
        { x: 464, y: 400 },
        { x: 496, y: 400 },
        { x: 528, y: 400 },
        { x: 464, y: 336 },
        { x: 1600, y: 400 },
        { x: 1632, y: 400 },
        { x: 1664, y: 336 },
        { x: 1696, y: 336 },
        { x: 2000, y: 400 },
        { x: 2032, y: 336 },
        { x: 2064, y: 272 }
    ],

    // Question blocks (contain power-ups or coins)
    questions: [
        { x: 352, y: 400, contains: 'coin' },
        { x: 560, y: 400, contains: 'mushroom' },
        { x: 1000, y: 336, contains: 'coin' },
        { x: 1632, y: 336, contains: 'coin' },
        { x: 2032, y: 400, contains: 'mushroom' },
        { x: 2500, y: 400, contains: 'coin' }
    ],

    // Coins placed in the level
    coins: [
        { x: 600, y: 300 },
        { x: 632, y: 300 },
        { x: 664, y: 300 },
        { x: 816, y: 350 },
        { x: 848, y: 350 },
        { x: 880, y: 350 },
        { x: 1216, y: 300 },
        { x: 1248, y: 300 },
        { x: 1516, y: 250 },
        { x: 1548, y: 250 },
        { x: 1800, y: 300 },
        { x: 2800, y: 300 },
        { x: 2832, y: 300 },
        { x: 2864, y: 300 }
    ],

    // Enemies (Goombas)
    enemies: [
        { type: 'goomba', x: 600, y: 500 },
        { type: 'goomba', x: 900, y: 500 },
        { type: 'goomba', x: 1100, y: 500 },
        { type: 'goomba', x: 1400, y: 500 },
        { type: 'goomba', x: 1700, y: 500 },
        { type: 'goomba', x: 1750, y: 500 },
        { type: 'goomba', x: 2200, y: 500 },
        { type: 'goomba', x: 2600, y: 500 },
        { type: 'goomba', x: 2650, y: 500 },
        { type: 'goomba', x: 3000, y: 500 }
    ],

    // Pipes
    pipes: [
        { x: 1200, y: 504, height: 64 },
        { x: 1800, y: 504, height: 64 },
        { x: 2400, y: 504, height: 96 },
        { x: 3100, y: 504, height: 64 }
    ],

    // Moving platforms (cross the gap between x=3200 and x=3800)
    movingPlatforms: [
        // First gap platforms (x=3200 to x=3800)
        { x: 3220, y: 500, width: 96, speed: 50 },  // First platform (slowest)
        { x: 3420, y: 460, width: 96, speed: 55 },  // Second platform (medium speed, lower)
        { x: 3620, y: 510, width: 96, speed: 60 },  // Third platform (fastest, higher)
        // Second gap platforms (x=4400 to x=5000)
        { x: 4450, y: 490, width: 96, speed: 52 },  // First platform in second gap
        { x: 4750, y: 520, width: 96, speed: 58 }   // Second platform in second gap
    ],

    // House (level transition to Level 2)
    house: {
        x: 6200,
        y: 472
    }
};
